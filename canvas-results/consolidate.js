...
for (var ua in data) {
    ...
    for (var i = 0, n = data[ua].results.length; i < n; i++) {
        ...
        for (var j = 0, m = testData.subtests.length; j < m; j++) {
            ...
            ,   nm = st.name
            ;
            if (nm === "constructor") nm = "_constructor";
            if (!out.results[id].subtests[nm]) out.results[id].subtests[nm] = { byUA: {}, totals: {} };
            out.results[id].subtests[nm].byUA[ua] = st.status;
            if (!out.results[id].subtests[nm].totals[st.status]) out.results[id].subtests[nm].totals[st.status] = 1;
            else out.results[id].subtests[nm].totals[st.status]++;
        }
    }
}
