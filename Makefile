...
output/html/single-page.html: source ../tools/*.py
	python ../tools/publish.py html
	cp -R images output/html/
	cp -R fonts output/html/
	cp 404/* output/html/

output/2dcontext/single-page.html: source ../tools/*.py
	python ../tools/publish.py 2dcontext
	cp -R images output/2dcontext/
	cp -R fonts output/2dcontext/

output/microdata/single-page.html: source ../tools/*.py
	python ../tools/publish.py microdata
